...
function getsolution(
    data::DataCVRP, optimizer::VrpOptimizer, x, objval, app::Dict{String,Any}
)
    ...
    for i in adj_list[1]
        if !visited[i]
            ...
            while next != 0 && it < maxit
                length(adj_list[next + 1]) != 2 && error(
                    "Problem trying to recover the route from the x values. " *
                ...
                visited[next] = true
                aux = next
                next, prev = if (adj_list[next + 1][1] == prev)
                    adj_list[next + 1][2]
                else
                    adj_list[next + 1][1]
                end,
    ...
    !isempty(filter(a->a==false, visited)) && error(
        "Problem trying to recover the route from the x values. " *
...
function checksolution(data::DataCVRP, solution)
    ...
    for (i, r) in enumerate(solution.routes)
        ...
        (sum_demand > Q) && error(
            "Route #$i is violating the capacity constraint. Sum of the demands is $(sum_demand) and Q is $Q",
        )
    end
    !isempty(filter(a->a==0, visits)) &&
        error("The following customers were not visited: $(filter(a->a==0,visits))")
    (abs(solution.cost-sum_cost) > 0.001) && error(
        "Cost calculated from the routes ($sum_cost) is different from that passed as" *
...
function readsolution(app::Dict{String,Any})
    ...
    error(
        "The solution file was not read successfully. The file must be in the CVRPLIB format.",
    )
    return sol
end
...
function drawsolution(tikzpath, data, solution)
    open(tikzpath, "w") do f
        ...
        for r in solution.routes
            ...
            for i in r
                e = (prev, i)
                edge_style = if (prev == 0)
                    "dashed,-,line width=0.2pt,opacity=.2"
                else
                    "-,line width=0.8pt"
                end
